...
<div class="form-group">
    <div class="row">
        <div class="col-3">
            <input type="text" class="form-control" name="kod" placeholder="Код"
                   required maxlength="15" value="{{ old('kod') ?? $product->kod ?? '' }}">
            </div>
            <div class="col-3">
                <input type="text" class="form-control" name="artikul" placeholder="Артикул"
                    required maxlength="30" value="{{ old('artikul') ?? $product->artikul ?? '' }}">
            </div>
    </div>
</div>
<div class="form-group">
...
<div class="form-group">
    ...
    <div class="row">
        <div class="col col-md-3">
            <input type="text" class="form-control w-100 d-inline mr-4" style="width: 20%" placeholder="Цена"
           name="price" required value="{{ old('price') ?? $product->price ?? '' }}">
        </div>
        <div class="col">
    <!-- валюта -->
            @php
              $valuta_id = old('valuta_id') ?? $product->valuta_id ?? 0;
            @endphp
            <select name="valuta_id" class="form-control w-100" title="Валюта">
                <option value="0">Выберите</option>
                @if(count($items))
                    @include('admin.product.part.valuta')
                @endif
            </select>
        </div>
        <div class="col col-md-7 flex-column">
    <!-- новинка -->
    ...
    <!-- Скидка -->
            <div class="form-check form-check-inline" style="width: 35%">
                ...
                <input type="text" class="form-control d-inline mr-4" placeholder="Скидка"
...
<div class="form-group">
    @isset($product->image) <img src="{{ $product->image }}"> @endisset
